...
import mao.aggregate_pay_user_api.fallback.ResourceFeignClientV2FallbackFactory;
 ...
 * Interface(接口名): ResourceFeignClientV2
 * Author(作者）: mao
...
@FeignClient(value = "aggregate-pay-user-service", path = "/user/v2", fallbackFactory = ResourceFeignClientV2FallbackFactory.class)
public interface ResourceFeignClientV2
{
    ...
    @PostMapping("/apps")
    R<Boolean> createApplication(@RequestBody ApplicationDTO application);
    ...
    @PutMapping("/apps")
    R<Boolean> modifyApplication(@RequestBody ApplicationDTO application);
    ...
    R<Boolean> removeApplication(@PathVariable String applicationCode);
    ...
    R<ApplicationDTO> queryApplication(@PathVariable String applicationCode);
    ...
    R<PageVO<ApplicationDTO>> pageApplicationByConditions(@RequestBody ApplicationQueryParams query, @RequestParam Integer pageNo, @RequestParam Integer pageSize);
    ...
    R<List<ResourceDTO>> loadResources(@RequestParam List<String> privileageCodes, @PathVariable String applicationCode);
    ...
    R<Map<Long, List<ResourceDTO>>> loadResources(@RequestBody Map<Long, AuthorizationInfoDTO> tenantAuthorizationInfoMap);
    ...
    @GetMapping("/menus/{id}")
    R<MenuDTO> queryMenu(@PathVariable Long id);
    ...
    R<List<MenuDTO>> queryMenuByApplicationCode(@PathVariable String applicationCode);
    ...
    R<PageVO<MenuDTO>> queryMenu(@RequestBody MenuQueryDTO params, @RequestParam Integer pageNo, @RequestParam Integer pageSize);
    ...
    R<List<MenuDTO>> queryMenuByPrivileges(@RequestParam String[] privileges);
}
